<template id="movies-now-playing-template">
  <div class="container" v-if="now_playing != null && genres != null">
    <h3 class="uppercase left-text">
      now playing
    </h3>
    <slick class="slick" ref="slick" :options="slickOptions">
      <div v-for="item in now_playing" :key="item.id">
        <router-link
          tag="button"
          class="button"
          :to="{
        ...
        <h5>
          {{ get_genre(genres, item.genre_ids) }}
        </h5>
      </div>
    </slick>
  </div>
...
export default {
  ...
  created() {
    axios
      .get(
        "https://api.themoviedb.org/3/movie/now_playing?api_key=f943d3d10cc39fd734122d69efabbacb"
      )
      .then((response) => {
