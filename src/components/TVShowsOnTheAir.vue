<template id="tvshows-on-the-air-template">
  <div class="container" v-if="on_the_air != null && genres != null">
    <h3 class="uppercase left-text">
      on the air
    </h3>
    <slick class="slick" ref="slick" :options="slickOptions">
      <div v-for="item in on_the_air" :key="item.id">
        <router-link
          tag="button"
          class="button"
          :to="{
...
export default {
  ...
  created() {
    axios
      .get(
        "https://api.themoviedb.org/3/tv/on_the_air?api_key=f943d3d10cc39fd734122d69efabbacb"
      )
      .then((response) => {
